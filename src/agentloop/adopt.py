...
conflict-aware. What it does (idempotent; --dry-run prints the plan only):
  ...
  copy   — .agentloop/ (state.md / tasks.yaml / config.yaml, plus prompts/ — the shared phase
           procedures), src/agentloop/, agentloop.mk, the per-agent wrappers
...
from agentloop import cycle, init
...
_EXCLUDE_FILE_PREFIXES = ("build-loop.log", "build-loop.lock", "events.ndjson", "adopt-manifest.yaml", "pr-draft.md")
