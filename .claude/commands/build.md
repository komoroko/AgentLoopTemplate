...
5. Once approved — a human explicitly approves (approving plan mode or an explicit "approve"); **running the next command (`/verify`) is not itself approval** — set `gates.build` to `approved`, `current_phase` to `verify`, and point to "next is `/verify`".
