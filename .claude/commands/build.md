...
5. **Pass the quality gate (definition of done / DoD).** The DoD is defined once, as `quality_gate.steps` in `.agentloop/config.yaml` — run those steps in order (default: `test` → `check` → `review` → `smoke`). When the task's own `test` command (tasks.yaml) differs from the configured steps, it runs **first** as a focused `task-test` step (mode A prepends it in code; in mode B the lead runs it the same way) — the ticket's green decision is executed, not just documented. Only `done` once every step passes:
