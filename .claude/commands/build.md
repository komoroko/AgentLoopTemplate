...
4. **Confirm tests green.** If still red, have the implementer fix it. If unsolvable within the set number of tries, set `blocked` and record it as an event (`make events ARGS='--add blocked --task T-NNN --detail "..."'`).
5. **Pass the quality gate (definition of done / DoD).** The DoD is defined once, as `quality_gate.steps` in `.agentloop/config.yaml` — run those steps in order (default: `test` → `check` → `review` → `smoke`). Only `done` once every step passes:
   ...
   - **Interactive mode (B): the lead re-runs each `cmd` step (`make test` / `make check`) itself before marking `done`** — a subagent/implementer's textual "green" report is not evidence, even when it pastes output (summarized/elided pastes have hidden real failures; mode A already gates on exit code in code). **Crucially, re-run on the *merged/integrated* state, not only the isolated task**: a leaf task can pass `make check` in its own worktree yet fail once cherry-picked/merged onto the work branch (a lint/format/type error only the *combined* file set surfaces, e.g. `ruff` across all files, or `ruff-format` on a construct another task's change reflows). So after each ascending-id merge (or after a batch of merges), re-run `make test` + `make check` on the work branch and fix any red the integration exposed **before** marking the merged tasks `done`. This lead re-run on the merged state is the effective backstop against green-report inaccuracy. (Deterministic mode A runs this same merged-state check in code — `build_loop.py`'s integration gate after each multi-leaf batch merge.)
...
7. If the implementer reports a **requirements/design defect**, set `needs-revision`; if a **new dependency or task split is discovered**, update the DAG (dependencies/dependents) in the task table. Log both (defects as events via `make events ARGS='--add …'`), and raise upstream defects to the human (do not fix on your own). **If an upstream deliverable needs fixing, use `/revise <phase>` at the human's discretion to roll back to requirements/design** (reset the gates in a chain to `pending` and analyze task impact with `dag.py --impacted`).
8. **Reassemble the chain**: reflect completions/changes, recompute the execution plan (layers / critical path / frontier) in `state.md`, update `updated_at`, and move to the next iteration. Newly freed tasks join the next iteration's frontier.
   - **Session hygiene (mode B only)**: at a layer boundary, if the conversation has grown heavy with re-run output, you may suggest the human run `/compact` — but only when no task is `in_progress`, completed tasks are merged/committed and marked `done`, and build observations are recorded in ticket Notes / `state.md` logs (pre-compact check: CLAUDE.md "Context budget"; the SSOT rehydrates the next iteration). Never mid-retry or while a worktree awaits its merge. Mode A runs in separate processes and does not need this.

## When all tasks complete (gate ④)
1. **Mandatorily run `/security-review`.** Review the work-branch diff for vulnerabilities, return must-fix-equivalent findings to the implementer to fix, and record judgment calls as escalation events (`make events ARGS='--add …'`; state.md's escalation view shows them) for the human. Do not present gate ④ if there is a serious unresolved issue.
...
3. Present the implementation summary (completed tasks, key additions/changes, test results, **security-review results**, unresolved items) and confirm "may we approve this as implementation-complete?".
   - **Smoke-step check**: if the deliverable is runnable (CLI, server, …) and `quality_gate`'s `smoke.run` is still empty, say so explicitly at the gate — the DoD ran without a launch check — and propose the command to fill in (an unnoticed empty smoke silently defeats its purpose).
