...
- **Leave a retrospective (recovering the metacognition)**: generate/update `docs/retrospective.md`.
  ...
  - **Upstream feedback (optional, also before `cycle-close`)**: if `github.feedback.enabled` is true in `.agentloop/config.yaml` and retrospective §4/§5 rows are marked `Promote? = upstream`, draft one issue per row into `.agentloop/feedback.yaml` (`issues:` list of `{title, body}`; self-contained English a template maintainer can act on without this repo's context — never include product internals or secrets) and show the drafts to the human. Filing is outward-facing and **human-run**: the human previews with `make feedback ARGS=--dry-run` and files with `make feedback`; record each filed issue URL in the row's "Promoted to (file)" column.
