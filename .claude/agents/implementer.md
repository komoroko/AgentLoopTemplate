...
- Once tests are green and the acceptance criteria are met, concisely report what you implemented and how, and which tests passed. **Report the actual command output / exit status of `make test` and `make check` — not a bare claim of "green".** The caller re-runs these independently to confirm, so an inaccurate green report will be caught; don't assert green you haven't observed.
