...
(ドロップダウン)が現れ、サーバを立て直さずにボードの対象を切り替えられる。`agentloop ui` は
起動元のリポジトリを常に自動登録する。単発なら `agentloop --repo <path> <verb>`(または
`AGENTLOOP_ROOT=<path>`)で、ディレクトリを移動せずに個々のコマンドを別リポジトリに向けられる。
...
- **Context** — コンテキストは必要最小限に保つ。真実は SSOT に置き、ロールエージェントは必要な
  ...
  恒久の3層で管理する(`.agentloop/prompts/rules/gate-workflow.md` の「Context budget」参照)。
...
```bash
uv tool install 'git+https://github.com/komoroko/AgentLoopTemplate.git@v0.9.0'   # `agentloop` コマンドが入る
```
...
そのロールの adapter を `.agentloop/config.yaml` に設定する。`gemini` や任意のコマンドも指定できる)。用意できない場合は
...
その新しいセッションで **`/req`** から始める(現在地と次の一手は常に `agentloop next` が示す——
「使い方」の節を参照)。

> **0.9.0 は破壊的な作り直し**で、0.8.x からの移行はない。`.agentloop/state.md`・`tasks.yaml`・
> `security-review.md` を持つリポジトリは `agentloop doctor --unsupported-layout` 以外の全 verb で
> 停止する。旧サイクルを archive し、`.agentloop/` を削除して `agentloop init` をやり直すこと。

`agentloop init` が書き込むのは**状態だけ**である:

- SSOT の4文書(`plan.yaml` / `state.yaml` / `review.yaml` / `config.yaml`、プレースホルダ入り)と、docs の
  スキャフォールド
- 実体化された `.agentloop/prompts`・`.agentloop/schema`・`.agentloop/AGENTS.agentloop.md`、
  初期スキャフォールドのスナップショット、`.agentloop/agentloop.lock`(ツールのバージョン・
...
`agentloop install` するまでは入らない。brownfield の場合は `/onboard` への案内も添えられる。

実体化されたファイルを最新に保つには:

- `agentloop sync` — インストール済みパッケージから prompts/schema を再実体化する(手を入れて
  いないファイルは更新し、ローカルで変更したファイルは保持して一覧表示。`--force` で上書き、
  `--check` は書き込まずズレの報告だけ)
- `agentloop upgrade` — CHANGELOG の差分を表示したうえで、ツールが実体化したものをすべて更新する
- `uv tool upgrade agentloop` — CLI 本体そのものを更新する
...
2. **デルタサイクル** — `brief → /req → … → /verify` の1周で**1つの変更**を扱い、
   `agentloop cycle-close` で締める(進め方は「使い方」と同じ)。`docs/00-product-brief.md` と
...
3. **ゲートを開く**: **署名済み Attestation でのみ**開く。`agentloop approve <gate>` は準備状態を
   確認して Attestation リクエストを出すだけで、ゲートは開かない。外部 Trust Manifest が名指す鍵が
   それに署名し、`agentloop attestation import <signed>` が receipt を記録する — これが `approved`
   への唯一の経路である。localhost のクリックは認証ではなく、`--force` は存在せず、ゲート行の手編集は
   ガードが拒否する。
4. **差し戻す**: 上流(要件・設計)の不備が見つかったら `/revise <phase>` を実行する。戻し先以降の
   ...
   する)。承認の巻き戻しも人間の判断で行う。
5. **進捗を確認する**:
   - `agentloop next` — 次に打つべきコマンドだけを表示する(連携用に `--json`)
   - `/status` — チャットで全体像を示す
   - `agentloop ui` — ダッシュボードを開く。Overview ボードに加え、**Review タブ**では承認待ち
     ゲートの成果物を読み、その場で承認まで完結できる(自己評価ブロックを上部にピン留め、
     ゲート④は Challenge-first の grounded review〈3軸と Coverage Manifest〉を diff とレビュー鮮度
     とともに表示)。Tasks タブ(DAG・レイヤー進行)、
   ...
   - `agentloop dag --mermaid` — タスクの依存図を生成する
...
- 実装完了時 — **構造化セキュリティレビュー**がゲート④の grounded review に組み込まれる。
  `agentloop review generate` がレビュー対象 HEAD に束ねて実行し、blocking な指摘はゲートを止める
  に残す
...
自動でスキップ)。`agentloop issue-sync` は plan のタスクを Issues へ**一方向にミラー**する —
...
- **まずは `agentloop doctor`** — 環境と SSOT を読み取り専用で一括診断する(PATH 上のバイナリ、
  ...
  エスカレーション、review の鮮度、Trust Manifest、sandbox の digest 固定、lock の健全性、schema 検証)。以下の症状の
  多くはここに FAIL / WARN として現れる。
- **タスクが `blocked` になった** — リトライ予算内で品質ゲートを通せなかったということ。
  `agentloop events --render` でエスカレーションの内容を読み、原因(またはタスク票)を直し、
  `state.yaml` の `status` を `todo` に戻し、`agentloop events --resolve <ID> --note "…"` で
...
| パス | 役割 |
|------|------|
| `.agentloop/plan.yaml` | 凍結された Expected Model: claim・source・evidence obligation・oracle・タスク DAG |
| `.agentloop/state.yaml` | 可変な状態: フェーズ・ゲート承認・タスク状態 |
| `.agentloop/review.yaml` | マシンレビューとヒューマンレビュー(それぞれ独立に digest 化) |
| `.agentloop/events.ndjson` | ハッシュ連鎖された監査ログ — あらゆる状態変更の機械可読な真実(`agentloop events` で操作。最初のイベント発生時に作られる) |
| `.agentloop/config.yaml` | 確定実行の設定と、DoD の唯一の定義(`quality_gate.steps`) |
| `.agentloop/attestations/` · `.agentloop/oracles/` | import した署名済み Attestation · 凍結された受け入れ Oracle バンドル |
| `.agentloop/agentloop.lock` | 文書フォーマット、ツールのバージョン・取得元、導入ファイルごとの内容ハッシュ |
| `.agentloop/schema/` | SSOT 各文書の JSON Schema(エディタでの検証と `agentloop doctor` が使う)— 実体化ファイル |
| `.agentloop/prompts/` | 全エージェントが読む共有のフェーズ手順・ロール定義・フェーズ限定ルールモジュール(`rules/`)— 実体化ファイル |
| `.agentloop/AGENTS.agentloop.md` | エージェントの入口が import する運用規約の本体 — 実体化ファイル |
| `AGENTS.md` / `CLAUDE.md` | エージェント中立な運用規約の正本 / Claude Code 向けの能力対応表(Claude Code が読むのは CLAUDE.md であり AGENTS.md ではない。`@AGENTS.md` インポートで規約を一度だけ読み込む。`agentloop install claude` が対応表ブロックと `.claude/` ラッパーを製品リポに書き込む) |
...
- エージェントの入口はオプトインで、`agentloop install claude|copilot` が書き込む。これらは
  インストール済みの `agentloop` CLI を呼び出すため、フックの前提として `uv tool install` が
  必要になる。
