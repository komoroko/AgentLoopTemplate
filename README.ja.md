...
5. **進捗を確認する**:
   ...
   - `agentloop ui` — ダッシュボードを開く。Overview ボードに加え、**Review タブ**では承認待ち
     ...
     ゲート④は diff とセキュリティレビューの鮮度も表示)。Tasks タブ(DAG・レイヤー進行)、
...
- 実装完了時 — **セキュリティレビュー**が必須。モード A では全タスク done の時点で自動的に
  ヘッドレス実行し、レビュー対象の HEAD を埋め込んだレポートを `.agentloop/security-review.md`
  に残す
- `/verify` — **セキュリティレビューと依存パッケージの脆弱性監査**が必須

`/security-review` を持たないエージェントは、同等の検査を行って同じ形式で記録する。
