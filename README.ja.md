...
日常の人間向けコマンドは、1 つの入口の 4 動詞に圧縮されている（それ以外はダッシュボードの
ボタンか運用系 `make` ターゲットの役割 — 一覧は `./agentloop --help`）:
...
**ヘッドレスのエージェント CLI** が必要 — 既定は `claude -p`、`./agentloop agent codex` で切替できる
（`.agentloop/config.yaml` の `build.headless.cmd` を書き換える。`gemini` や任意コマンドも可）。
無ければ対話のモード B を使う（「エージェント対応」参照）。
...
# 3. プロダクトとして初期化 — 対話ウィザード（推奨。名前 / ブランチ / 更新取得元 /
#    ヘッドレスCLI / product-brief の最初の1行を質問される）
./agentloop start
# 非対話でやるなら（冪等）:
...
`make init`（ウィザードも同じ手順を実行する）はプレースホルダを埋め、作業ブランチを作成・切替し
（実装は main 直ではなくそこで行う）、
...
5. **進捗確認**: `./agentloop next` が次に打つべきコマンドだけを表示（連携用に `--json`）、`/status` は
   チャットで全体像、`make ui`（= `./agentloop ui`）は同じ内容をブラウザで見られる（既定で読み取り専用。
   安全な操作の固定ホワイトリストとゲート承認の記録もページから実行できる）。タスクの依存図は
   `uv run --no-project --with pyyaml python scripts/agentloop/dag.py --mermaid` で生成できる。
...
| `agentloop` | 日常の人間向け入口 — `./agentloop start / next / ui / agent`（uv のみ） |
