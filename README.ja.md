...
1. **`/onboard`** — 既存コードベースを読み取り専用で調査し、**永続ベースライン** `docs/05-current-state.md` を生成する: アーキテクチャ・モジュールの役割・再利用可能な資産・規約・既存ドキュメントへのリンク（移動・変換はせず現在地のまま）・実装状況（仕掛かり作業を含む）。既存の動作を要件や done タスクへ**逆生成はしない** — ゲートを開くのは常に人間で、トレーサビリティ（R-N）は各サイクルのデルタにだけ適用される。承認済み相当の要件書・設計書が既にあるなら、`/req`・`/design` をその取り込みとして高速に走らせてゲートを開けばよい — その承認こそが本機構への対応付けになる。
2. **デルタサイクル** — `brief → /req → … → /verify` の1周は**1つの変更**を扱い、仕掛かりの残作業はデルタ要件として再開する。リリース判断のあと、サイクルを閉じる:
   ```bash
