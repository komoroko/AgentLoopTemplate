...
前提: WSL / Linux / macOS と `make`（Windows ネイティブ不可）。確定ビルドループ（`make build-loop`、モード A）は加えて `claude` CLI の導入と認証が必要（implementer と review ステップが headless `claude -p` を回すため。Claude Code 専用 — 他のエージェントからは対話のモード B を使う。「エージェント対応」参照）。
...
オーケストレータを使わず会話でループを回す代替（Claude Code 以外で使える唯一のモード）。Claude Code は `/loop /build` で、VS Code Copilot は `/build` プロンプトを反復起動して、Codex は `/build` の手順を再実行して回す。
...
| ゲート強制（機構レイヤー） | PreToolUse フック（`gate_guard.py`） | 同じフックを `.github/hooks/agentloop.json` 経由で（agent hooks、preview） | 慣習レイヤーのみ（フックがファイル編集を横取りできない） |
...
| ヘッドレスのビルドオーケストレータ（モード A） | `make build-loop`（`claude -p`） | —（モード B を使う） | —（モード B を使う） |
...
フェーズ名の指示（「/req を実行」）で駆動できる。ゲート強制は慣習のみ（Codex のフックは Bash は
横取りできるがファイル編集はできない）、セキュリティレビューは同等の手動パスで代替する —
どのフックホストが登録済みかは `make doctor` が報告する。
