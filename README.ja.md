...
| `scripts/agentloop/` | 確定オーケストレーション（`dag.py`／`build_loop.py`／`events.py`／`doctor.py`／`gate_guard.py`／`pr_draft.py`／`revise.py`／`issue_sync.py`／`feedback.py`／`init.py`／`adopt.py`／`cycle.py`）。プロダクト用は `scripts/` 直下 |
