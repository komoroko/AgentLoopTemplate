...
要件定義からテストまでの作業・成果物の作成・自己テストはエージェントが担当し、
**人間は各フェーズの境界にある「ゲート」で承認・判断するだけ**でよい、という開発の枠組みを提供する。

ハーネスの本体は**インストールして使う CLI**(`agentloop`)で、プロダクトのリポジトリ側に残るのは
*状態*だけ: `.agentloop/`(SSOT〈信頼できる唯一の情報源〉・lock・実体化された prompts/schema)と
`docs/`(フェーズ成果物)である。
...
- **Context** — コンテキストは必要最小限に保つ。真実は SSOT に置き、ロールエージェントは必要な
  分だけ読む。失敗はダンプせず要約し、ログは自動ローテーション、記憶はセッション/サイクル/恒久の
  3層で管理する(`AGENTS.md` の「Context budget」参照)。
...
# 対話ウィザード(推奨。プロダクト名 / ブランチ / 取得元 / ヘッドレス CLI / brief の1行を質問される)
agentloop start
# 非対話で行う場合(何度実行しても安全):
...
`tasks.yaml`、プレースホルダ入り)、docs のスキャフォールド、実体化された `.agentloop/prompts`・
`.agentloop/schema`・`.agentloop/AGENTS.agentloop.md`、初期スキャフォールドのスナップショット、
そして `.agentloop/agentloop.lock`(ツールのバージョン・取得元と、導入ファイルごとの内容ハッシュ)。
...
3つの層で担保する: **gitleaks**(pre-commit でシークレットのコミットを防ぐ。誤検知は
`.gitleaksignore` へ)/ 実装完了時の**セキュリティレビュー**(必須。モード A では全タスク done の
時点で自動的にヘッドレス実行し、レビュー対象の HEAD を埋め込んだレポートを
`.agentloop/security-review.md` に残す)/ `/verify` での**セキュリティレビュー + 依存パッケージの
脆弱性監査**(必須)。`/security-review` を持たないエージェントは、同等の検査を行って同じ形式で
記録する。
