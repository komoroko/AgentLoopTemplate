...
## How to proceed
1. Read the specified `docs/tasks/T-NNN.md` and, in `docs/20-design.md`, **the design section(s) covering your task's requirement (`req: R-x`)** — do not load the whole design doc beyond what your task needs (keeping the read lean avoids *Lost in the Middle*; see `.agentloop/prompts/rules/gate-workflow.md` "Context budget"). Then read the existing code.
