...
3. **Task progress**: run `agentloop dag --render` and show its deterministic output (counts, execution layers, critical path, executable frontier). Also list `blocked`/`needs-revision` tasks individually (they need human intervention). Skip if tasks.yaml is not generated yet (before `/tasks`).
...
6. **(Only with GitHub integration)** If `github.enabled: true` in `.agentloop/config.yaml`, give a one-line note that `agentloop issue-sync` can bring Issues into line with this dashboard (tasks.yaml) (Issues are a one-way mirror, not the SSOT).
