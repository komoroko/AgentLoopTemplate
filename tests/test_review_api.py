...
class TestGateMapping:
    def test_unknown_gate_raises(self, make_repo) -> None:
        root = make_repo()
        with pytest.raises(review_api.ReviewError):
            review_api.collect_review(root, "nope")

    def test_requirements_gate_deliverable_and_context(self, make_repo) -> None:
        root = make_repo(
        ...
        out = review_api.collect_review(root, "requirements")
    ...
    def test_unfilled_confidence_placeholder_reads_unset(self, make_repo) -> None:
        root = make_repo()
        _write(root, "docs/10-requirements.md", "## Self-assessment\n- **Confidence**: high / medium / low\n")
        (main,) = review_api.collect_review(root, "requirements")["deliverables"]
        assert main["self_assessment"]["confidence"] is None

    def test_design_glob_excludes_template_and_sorts(self, make_repo) -> None:
        root = make_repo()
        for name in ("ADR-002.md", "ADR-001.md", "ADR-template.md"):
            _write(root, f"docs/decisions/{name}", f"# {name}")
        labels = [d["label"] for d in review_api.collect_review(root, "design")["deliverables"]]
        assert labels == ["docs/20-design.md", "docs/decisions/ADR-001.md", "docs/decisions/ADR-002.md"]

    def test_tasks_gate_renders_tickets_and_verbatim_yaml(self, make_repo) -> None:
        ...
        out = review_api.collect_review(root, "tasks")
    ...
    def test_missing_deliverable_is_reported_absent(self, make_repo) -> None:
        root = make_repo()
        (main,) = review_api.collect_review(root, "requirements")["deliverables"]
        assert main["exists"] is False and main["html"] == ""

    def test_release_gate_counts_open_escalations(self, make_repo) -> None:
        root = make_repo()
        line = {"id": 1, "ts": "2026-07-19T00:00:00", "event": "blocked", "task": "T-001", "detail": "x"}
        _write(root, ".agentloop/events.ndjson", json.dumps(line) + "\n")
        assert review_api.collect_review(root, "release")["open_escalations"] == 1


class TestReachSafety:
    def test_symlink_escaping_the_repo_reads_absent(self, make_repo, tmp_path_factory) -> None:
        root = make_repo()
    ...
    def test_oversize_deliverable_is_truncated(self, make_repo) -> None:
        root = make_repo()
        _write(root, "docs/10-requirements.md", "x" * (review_api._MAX_DELIVERABLE + 100))
        (main,) = review_api.collect_review(root, "requirements")["deliverables"]
        assert main["truncated"] is True

    def test_agent_markup_never_survives_rendering(self, make_repo) -> None:
        root = make_repo()
...
class TestBuildGateDiff:
    def test_non_git_repo_degrades_to_error(self, make_repo) -> None:
        out = review_api.collect_review(make_repo(), "build")
        assert "error" in out["diff"]
        assert out["review_meta"]["fresh"] is False

    def test_branch_diff_against_merge_base(self, make_repo) -> None:
        root = make_repo()
        ...
        out = review_api.collect_review(root, "build")
        diff = out["diff"]
    ...
    def test_head_at_base_falls_back_to_log(self, make_repo) -> None:
        root = make_repo()
        _git(root, "init", "-q", "-b", "main")
        _write(root, "a.txt", "base\n")
        _git(root, "add", ".")
        _git(root, "commit", "-qm", "only commit")
        diff = review_api.collect_review(root, "build")["diff"]
        assert "patch" not in diff
        assert diff["log"] and "only commit" in diff["log"][0]

    def test_security_review_freshness(self, make_repo) -> None:
        root = make_repo()
        _git(root, "init", "-q", "-b", "main")
        _write(root, "a.txt", "x\n")
        _git(root, "add", ".")
        ...
        assert review_api.collect_review(root, "build")["review_meta"]["fresh"] is True
        _write(root, ".agentloop/security-review.md", "Reviewed-HEAD: 0000000\n")
        meta = review_api.collect_review(root, "build")["review_meta"]
